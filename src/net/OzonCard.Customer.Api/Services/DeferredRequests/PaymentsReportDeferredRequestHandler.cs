...
public class PaymentsReportDeferredRequestHandler : IDeferredRequestsHandler
{
    private readonly ILogger<PaymentsReportDeferredRequestHandler> _logger;
    private readonly IBackgroundJobsService _jobsService;
    ...
    public PaymentsReportDeferredRequestHandler(
    ...
    {
        _logger.LogDebug("Append new PaymentsReportDeferredRequestHandler");
        var cmd = new ReportPaymentsCommand()
        {
            Title = configuration.GetProperty<ExtensionStringProperty>("title").Value,
            OrganizationId = configuration.GetProperty<ExtensionGuidProperty>("organizationId").Value,
            ProgramId = configuration.GetProperty<ExtensionGuidProperty>("programId").Value,
            Batch = configuration.GetProperty<ExtensionGuidProperty>("batchId").Value,
        ...
        return Task.FromResult(_jobsService.Schedule(cmd, configuration.Schedule, cmd.Tracking));
    }
}