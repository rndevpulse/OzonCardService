...
public class RemoveCategoriesDeferredRequestHandler(
    ILogger<RemoveCategoriesDeferredRequestHandler> logger,
    IBackgroundJobsService jobsService
) : IDeferredRequestsHandler
{
    public string Key => DRequests.Categories.Remove;
    public string Name => "Удалить категорию у пользователей";
    ...
    private IEnumerable<ExtensionProperty> GetProps() =>
        new List<ExtensionProperty>()
        {
            ...
            new ExtensionGuidProperty("selectedCategory", "Удаляемая категория", PropertyBehaviour.CategoryId),
            
        };
    
    public Task<IBackgroundTask> AppendAsync(RequestConfiguration configuration, CancellationToken ct = default)
    {
        logger.LogDebug("Append new RemoveCategoriesDeferredRequestHandler");
